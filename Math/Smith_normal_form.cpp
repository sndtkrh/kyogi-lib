...
typedef vector< vector<long long> > Matrix; // 行列型

void swap_row(Matrix &M, int s, int t){ // 列を交換
  if( s == t ) return;
  for(int j = 0; j < M[s].size(); j++) swap( M[s][j], M[t][j] );
}

vector<long long> compute_snf(Matrix M){
  int n = M.size();
  int m = M[0].size();
  ...
  for(int i = 0; i < diag.size() - 1; i++){
    if( diag[i+1] % diag[i] != 0 ){
      int g = __gcd( diag[i], diag[i+1] ); // gcd__(x,y) はgcc拡張機能
      int p = diag[i];
      diag[i] = g;
      diag[i+1] *= -p / g;
    }
    diag[i] = abs( diag[i] ); // 正にしておく
  }
...
int main(){
  Matrix M = {{2,4,4},
	      {-6,6,12},
	      {10,-4,-16}};
  vector<long long> d = compute_snf(M);
